	...
	<?foreach($arResult["ITEMS"] as $arItem):?>
		<div itemscope itemtype="http://schema.org/Event" class="board-item-holder"><a href="<?=$arItem["DETAIL_PAGE_URL"]?>" itemprop="url" class="board-item">
		  <div class="board-item-inner">
			...
			?>
			<div style="background-image: url(<?=$arPhotoPreview['src']?>)" class="board-item-photo-holder"><img itemprop="image" src="<?=$arPhotoPreview['src']?>" alt="<?=$arItem["NAME"]?>" width="350" height="350" class="board-item-photo"><!--noindex--><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" alt="" width="100%" class="board-item-photo-spacer"><!--/noindex--></div>
			<div class="board-item-info">
			  ...
			  <?}?>
			  <?if(count($arItem['DISPLAY_PROPERTIES']['PHOTO_LIST']['VALUE']) || $arItem['DISPLAY_PROPERTIES']['VIDEO']['VALUE']){?>
				  <div class="board-item-media">
					<?if(count($arItem['DISPLAY_PROPERTIES']['PHOTO_LIST']['VALUE'])){?>
						<span title="Фотографий" class="board-item-media-item camera-icon-before"><span class="board-item-media-item-counter"><?=count($arItem['DISPLAY_PROPERTIES']['PHOTO_LIST']['VALUE'])?></span></span>
					<?}?>
